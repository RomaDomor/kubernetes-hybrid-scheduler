...
def plot_7_raw_data_variance(df: pd.DataFrame, output_dir: Path, config_name: str):
    ...
    ax.legend(handles[:len(plot_df['local_load'].unique())], labels[:len(plot_df['local_load'].unique())],
              title='Local Load')
...
def plot_8_placement_analysis(df: pd.DataFrame, output_dir: Path, config_name: str):
    ...
    g.figure.legend(handles, ['cloud', 'edge'], title='Node Type', loc='upper center', bbox_to_anchor=(0.5, -0.02),
                    ncol=2)
...
def plot_compare_overall_slo_rate(df: pd.DataFrame, output_dir: Path):
    """Compares the average SLO pass rate across all configurations."""
    print("  - Generating: COMP_1. Overall SLO Pass Rate Comparison")
    if df.empty: return

    slo_rate = df.groupby(['config_name', 'wan_profile', 'local_load'], observed=True)['pass'].value_counts(
...
def plot_compare_key_job_performance(df: pd.DataFrame, output_dir: Path, workload: str):
    """Compares the performance of a single key workload across configurations."""
...
def plot_compare_placement_stability(df: pd.DataFrame, output_dir: Path):
    """Compares the placement decision stability across configurations."""
    ...
    majority_placement = (
        df_clean.groupby(['config_name', 'wan_profile', 'local_load', 'run', 'workload', 'node_type'], observed=True)
        .size().reset_index(name='count'))
    idx = majority_placement.groupby(['config_name', 'wan_profile', 'local_load', 'run', 'workload'], observed=True)[
        'count'].idxmax()
    majority = majority_placement.loc[
        idx, ['config_name', 'wan_profile', 'local_load', 'run', 'workload', 'node_type']].copy()
    majority = majority.sort_values(['config_name', 'wan_profile', 'local_load', 'workload', 'run'])

    majority['prev_node_type'] = \
    majority.groupby(['config_name', 'wan_profile', 'local_load', 'workload'], observed=True)['node_type'].shift(1)
    majority['switched'] = (majority['node_type'] != majority['prev_node_type']) & majority['prev_node_type'].notna()

    switches = majority.groupby(['config_name', 'wan_profile', 'local_load'], observed=True)[
        'switched'].sum().reset_index(name='total_switches')
...
def main():
    ...
    for config_name in df_stable_slo['config_name'].unique():
        print(f"\n--- Generating plots for configuration: {config_name} ---")
        ...
        df_slo_config_stable = df_stable_slo[df_stable_slo['config_name'] == config_name]
        df_placement_config_stable = df_stable_placement[df_stable_placement['config_name'] == config_name]
        df_slo_config_all = df_all_slo[df_all_slo['config_name'] == config_name]
        df_placement_config_all = df_all_placement[df_all_placement['config_name'] == config_name]

        # --- Stable-State Performance Plots ---
        ...
        # --- Learning Curve & Advanced Plots (using all data) ---
        # plot_10_learning_performance_over_runs uses a single config's data
    ...
    # These plots are inherently comparative, so they go in the comparative section
    ...
    plot_12_placement_performance_correlation(df_all_slo, df_all_placement, comparative_output_dir)
    plot_13_workload_preference_heatmap(df_stable_placement, comparative_output_dir)
