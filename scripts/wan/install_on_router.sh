...
#     --edge-cidr 10.10.0.0/24 --cloud-cidr 10.20.0.0/24 --fog-cidr 10.30.0.0/24 \
#     [--profile moderate]
#
...
if [[ -z "$FOG_CIDR" ]]; then
  echo "ERROR: --fog-cidr is required (e.g. --fog-cidr 10.30.0.0/24)" >&2
  exit 1
fi
...
# Find the router interface whose IP belongs to the given network CIDR
iface_by_network() {
  local cidr="$1"
  local network="${cidr%/*}"
  local prefix="${cidr#*/}"
...
if [[ -n "$FOG_CIDR_IN" ]]; then
  fog_ip="${FOG_CIDR_IN%%/*}"
  fog_if="$(iface_by_network "$FOG_CIDR_IN" || true)"
fi

# Auto-guess from ethernet-like interfaces if detection failed
mapfile -t CANDS < <(sudo -n ip -o -4 addr show | awk '{print $2}' | grep -E '^(en|eth|ens|enp|eno)' | sort -u)
if [[ -z "$edge_if"  ]] && [[ "${#CANDS[@]}" -ge 1 ]]; then edge_if="${CANDS[0]}";  fi
if [[ -z "$cloud_if" ]] && [[ "${#CANDS[@]}" -ge 2 ]]; then cloud_if="${CANDS[1]}"; fi
if [[ -z "$fog_if"   ]] && [[ "${#CANDS[@]}" -ge 3 ]]; then fog_if="${CANDS[2]}";   fi

if [[ -z "$edge_if" || -z "$cloud_if" || -z "$fog_if" ]]; then
  echo "ERROR: Failed to detect three distinct interfaces for EDGE_IF, CLOUD_IF, FOG_IF." >&2
  echo "Detected so far: EDGE_IF='$edge_if' CLOUD_IF='$cloud_if' FOG_IF='$fog_if'" >&2
  sudo -n ip -o -4 addr show >&2 || true
  exit 1
fi

if [[ "$edge_if" == "$cloud_if" || "$edge_if" == "$fog_if" || "$cloud_if" == "$fog_if" ]]; then
  echo "ERROR: Detected duplicate interfaces: EDGE=$edge_if CLOUD=$cloud_if FOG=$fog_if" >&2
  exit 1
fi
...
export FOG_IF="$fog_if"
export EDGE_CIDR="${EDGE_CIDR_IN}"
export CLOUD_CIDR="${CLOUD_CIDR_IN}"
export FOG_CIDR="${FOG_CIDR_IN}"
export EDGE_IP="${edge_ip}"
export CLOUD_IP="${cloud_ip}"
export FOG_IP="${fog_ip}"
export PATH="/usr/local/sbin/wan:\$PATH"
EOF

echo "Detected EDGE_IF=$edge_if CLOUD_IF=$cloud_if FOG_IF=$fog_if"
RS
  scp "$TMP_LOCAL" "$ROUTER":/tmp/wan_detect.sh
  rm -f "$TMP_LOCAL"
  set +e
  REMOTE_OUT="$(ssh -tt "$ROUTER" \
    "EDGE_CIDR_IN='$EDGE_CIDR' CLOUD_CIDR_IN='$CLOUD_CIDR' FOG_CIDR_IN='$FOG_CIDR' bash /tmp/wan_detect.sh" 2>&1)"
  RC=$?
  set -e
...
validate_detection() {
  ...
  ssh "$ROUTER" "set -e; . /etc/wan/env; \
    sudo -n ip -o -4 addr show dev \"\$EDGE_IF\"; \
    sudo -n ip -o -4 addr show dev \"\$CLOUD_IF\"; \
    sudo -n ip -o -4 addr show dev \"\$FOG_IF\"; true"
}
...
echo "[*] Done. Router is ready (EDGE↔CLOUD and EDGE↔FOG links configured)."
