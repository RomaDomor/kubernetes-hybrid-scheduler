#!/usr/bin/env bash
# scripts/wan/install_on_router.sh
# Usage:
#   ./scripts/wan/install_on_router.sh user@router \
#     --edge-cidr 10.10.0.1/24 --cloud-cidr 10.20.0.1/24 [--profile moderate]
#
# Requires passwordless SSH or an agent. Adjust as needed.

set -euo pipefail

# Resolve directory of this script (handles symlinks and relative paths)
SCRIPT_SRC="${BASH_SOURCE[0]}"
while [ -h "$SCRIPT_SRC" ]; do
  SCRIPT_DIR="$(cd -P "$(dirname "$SCRIPT_SRC")" && pwd)"
  SCRIPT_SRC="$(readlink "$SCRIPT_SRC")"
  [[ "$SCRIPT_SRC" != /* ]] && SCRIPT_SRC="$SCRIPT_DIR/$SCRIPT_SRC"
done
SCRIPT_DIR="$(cd -P "$(dirname "$SCRIPT_SRC")" && pwd)"

ROUTER="${1:?SSH target, e.g., user@10.0.0.1}"
shift || true

EDGE_CIDR=""
CLOUD_CIDR=""
PROFILE=""

while [[ $# -gt 0 ]]; do
  case "$1" in
    --edge-cidr) EDGE_CIDR="${2:?}"; shift 2 ;;
    --cloud-cidr) CLOUD_CIDR="${2:?}"; shift 2 ;;
    --profile) PROFILE="${2:?good|moderate|poor|clear}"; shift 2 ;;
    *) echo "Unknown arg: $1" >&2; exit 1 ;;
  esac
done

copy_scripts() {
  echo "[*] Copying WAN scripts to $ROUTER ..."
  ssh "$ROUTER" "sudo mkdir -p /usr/local/sbin/wan /etc/wan"
  # Copy all .sh files that are in the same dir as this installer
  scp "$SCRIPT_DIR"/*.sh "$ROUTER":/tmp/
  ssh "$ROUTER" "sudo mv /tmp/*.sh /usr/local/sbin/wan/ && sudo chmod +x /usr/local/sbin/wan/*.sh"
}

detect_ifaces_on_router() {
  echo "[*] Detecting interfaces on router ..."
  # Build remote detection script
  read -r -d '' REMOTE <<'RS'
set -euo pipefail

EDGE_CIDR_IN="${EDGE_CIDR_IN:-}"
CLOUD_CIDR_IN="${CLOUD_CIDR_IN:-}"

edge_ip=""
cloud_ip=""
edge_if=""
cloud_if=""

# Helper: get interface by IPv4 address
iface_by_ip() {
  local ip="$1"
  ip -o -4 addr show | awk -v ip="$ip" '$0 ~ ip {print $2; exit}'
}

if [[ -n "$EDGE_CIDR_IN" ]]; then
  edge_ip="${EDGE_CIDR_IN%%/*}"
  edge_if="$(iface_by_ip "$edge_ip" || true)"
fi
if [[ -n "$CLOUD_CIDR_IN" ]]; then
  cloud_ip="${CLOUD_CIDR_IN%%/*}"
  cloud_if="$(iface_by_ip "$cloud_ip" || true)"
fi

# Auto-guess if still empty: pick two distinct ethernet-like ifaces
if [[ -z "$edge_if" || -z "$cloud_if" || "$edge_if" == "$cloud_if" ]]; then
  # List candidate ifaces with IPv4 addresses, prefer "ens|eth|enp"
  mapfile -t CANDS < <(ip -o -4 addr show | awk '{print $2}' | grep -E '^(en|eth|ens|enp|eno)' | sort -u)
  if [[ "${#CANDS[@]}" -ge 2 ]]; then
    # If one of them matches provided IPs (if any), keep it
    if [[ -z "$edge_if" && -n "$edge_ip" ]]; then
      edge_if="$(iface_by_ip "$edge_ip" || true)"
    fi
    if [[ -z "$cloud_if" && -n "$cloud_ip" ]]; then
      cloud_if="$(iface_by_ip "$cloud_ip" || true)"
    fi
    # Fill remaining by picking first two distinct
    if [[ -z "$edge_if" ]]; then edge_if="${CANDS[0]}"; fi
    if [[ -z "$cloud_if" ]]; then
      for i in "${CANDS[@]}"; do
        if [[ "$i" != "$edge_if" ]]; then cloud_if="$i"; break; fi
      done
    fi
  fi
fi

if [[ -z "$edge_if" || -z "$cloud_if" || "$edge_if" == "$cloud_if" ]]; then
  echo "ERROR: Failed to detect two distinct interfaces for EDGE_IF and CLOUD_IF." >&2
  ip -o -4 addr show >&2 || true
  exit 1
fi

# Persist env
sudo mkdir -p /etc/wan
cat <<EOF | sudo tee /etc/wan/env >/dev/null
# Autogenerated by install_on_router.sh
export EDGE_IF="$edge_if"
export CLOUD_IF="$cloud_if"
export EDGE_CIDR="${EDGE_CIDR_IN}"
export CLOUD_CIDR="${CLOUD_CIDR_IN}"
export EDGE_IP="${edge_ip}"
export CLOUD_IP="${cloud_ip}"
export PATH="/usr/local/sbin/wan:\$PATH"
EOF

echo "Detected EDGE_IF=$edge_if CLOUD_IF=$cloud_if"
RS

  # Run remote detection with provided CIDRs (if any)
  ssh "$ROUTER" \
    "EDGE_CIDR_IN='$EDGE_CIDR' CLOUD_CIDR_IN='$CLOUD_CIDR' bash -s" <<<"$REMOTE"
}

validate_detection() {
  echo "[*] Validation: show qdisc and ifaces"
  ssh "$ROUTER" "set -e; . /etc/wan/env; ip -o -4 addr show dev \$EDGE_IF; ip -o -4 addr show dev \$CLOUD_IF; true"
}

apply_optional_profile() {
  if [[ -n "$PROFILE" ]]; then
    echo "[*] Applying profile '$PROFILE' ..."
    ssh "$ROUTER" "set -e; . /etc/wan/env; /usr/local/sbin/wan/apply_wan.sh '$PROFILE'"
  else
    echo "[*] Skipping profile apply (none requested)."
  fi
}

copy_scripts
detect_ifaces_on_router
validate_detection
apply_optional_profile

echo "[*] Done. Router is ready. You can now run:"
echo "    ssh $ROUTER '. /etc/wan/env; /usr/local/sbin/wan/apply_wan.sh good|moderate|poor|clear'"