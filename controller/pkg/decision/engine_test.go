...
func TestDecide_NilLocalState_ForcesCloud(t *testing.T) {
	e := newEngine()
	p := podWith("latency", 200, 128)
	s := sloMust("latency", 2000, true, 5)
	wan := &apis.WANState{RTTMs: 50}

	res := e.Decide(p, s, nil, wan)
	if res.Location != constants.Cloud || res.Reason != constants.ReasonTelemetryCircuitBreaker {
		t.Fatalf("want CLOUD with telemetry_circuit_breaker, got %v %s", res.Location, res.Reason)
	}
}

func TestDecide_NilWANState_ForcesEdge(t *testing.T) {
	e := newEngine()
	p := podWith("latency", 200, 128)
	s := sloMust("latency", 2000, true, 5)
	local := localStateWithDefaults()

	res := e.Decide(p, s, local, nil)
	if res.Location != constants.Edge || res.Reason != constants.ReasonWANCircuitBreaker {
		t.Fatalf("want EDGE with wan_circuit_breaker, got %v %s", res.Location, res.Reason)
	}
}

// --- NEW: Tests for Feasibility Logic (isEdgeFeasibleWithConfidence) ---
...
func TestDecide_InfeasibleResources_Hard_TooLargeForNode(t *testing.T) {
	...
	if res.Location != constants.Cloud || res.Reason != constants.ReasonCloudFeasibleOnly {
		t.Fatalf("Expected CLOUD with cloud_feasible_only because pod is too big for edge, got %v %s", res.Location, res.Reason)
	}
}
...
func TestDecide_InfeasibleResources_Hard_UtilizationLimit(t *testing.T) {
	...
	res := e.Decide(p, s, local, wan)
	if res.Location != constants.Cloud || res.Reason != constants.ReasonCloudFeasibleOnly {
		t.Fatalf("Expected CLOUD with cloud_feasible_only due to utilization limit, got %v %s", res.Location, res.Reason)
	}
}
...
func TestDecide_InfeasibleTime_Edge(t *testing.T) {
	...
	ps.Update(edgeKey, apis.ProfileUpdate{ObservedDurationMs: 1200}) // P95 > 1000ms deadline
	...
	res := e.Decide(p, s, local, wan)
	if res.Location != constants.Cloud || res.Reason != constants.ReasonCloudFeasibleOnly {
		t.Fatalf("Expected CLOUD with cloud_feasible_only when edge is time-infeasible, got %v %s", res.Location, res.Reason)
	}
}
...
func TestDecide_InfeasibleResources_Soft(t *testing.T) {
	...
	res := e.Decide(p, s, local, wan)
	if res.Location != constants.Cloud || res.Reason != constants.ReasonCloudFeasibleOnly {
		t.Fatalf("Expected CLOUD with cloud_feasible_only when edge is temporarily full, got %v %s", res.Location, res.Reason)
	}
}
...
func TestDecide_LyapunovAdaptsToViolations(t *testing.T) {
	...
	ps.Update(edgeKey, apis.ProfileUpdate{ObservedDurationMs: 900})
	...
	ps.Update(edgeKey, apis.ProfileUpdate{ObservedDurationMs: 1200})
	...
	if res2.Location != constants.Cloud {
		t.Errorf("Expected CLOUD after many edge violations, but got %s", res2.Location)
	}
}
